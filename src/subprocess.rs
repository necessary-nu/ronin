...
pub struct ProcessCompletion {
    pub edge: EdgeId,
    pub result: Result<Option<ProcessOutput>, String>,
}
...
impl ProcessSupervisor {
    pub fn spawn<'scope, 'environment: 'scope>(
        &mut self,
    ...
    pub fn interrupt(&mut self, signal: i32) {
...
pub fn status_interrupted(status: &std::process::ExitStatus) -> bool {
    #[cfg(unix)]
    {
    ...
    pub fn do_work(&mut self) -> io::Result<WorkResult> {
        ...
        let (exit, output) = {
            ...
            let result = run_shell(
                &BString::from(process.command.as_bytes()),
                process.use_console,
                false,
                |_pid, _process_group| {},
            )?
            .expect("non-dry-run subprocesses produce output");
            let exit = classify_exit(&result.status);
