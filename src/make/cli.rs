...
use pass::{pass_session, record_invocation_variables, session_for};
...
impl Invocation {
    /// The `VAR=value` words this invocation was given, in command-line order.
    pub(crate) fn variables(&self) -> &[Bytes] {
        &self.variables
    }

    /// The goals this invocation's own command line named.
...
fn prepare_graph(
    ...
    while fruitless < MANIFEST_RETRY_LIMIT {
        ...
        let (session, compilation) = pass_session(root, restarts, &scripts);
