...
/// A recipe naming a depfile is read where it is built, not held for the launch
/// the way an ordinary, a `$?`, or a grouped `&::` recipe is. The depfile is a
/// dependency read at runtime and the edge that reads it has to declare it — a
/// deferred rule has no path to — so the recipe is expanded at construction and
/// its `$(file ...)` is performed there, even for a target that is up to date.
/// This gate pins both halves of the recorded decision in
/// docs/make-oracle-divergences.md: the depfile dependency the reading buys
/// (which deferring would drop — the tooth), and the `$(file ...)` write at
/// construction that reading costs (which a sound deferral would take away —
/// the other tooth, reopening the decision).
#[cfg(feature = "make")]
#[test]
fn a_depfile_recipe_is_read_where_built() {
    let run_in = |dir: &Path| {
        Command::new(dir.join("make"))
            .current_dir(dir)
            .env_remove("MAKEFLAGS")
            .env_remove("MFLAGS")
            .env_remove("CARGO_MAKEFLAGS")
    ...
    // A prerequisite aged into the past so the target a build makes now is
    // unambiguously newer than it, and a later rewrite unambiguously newer than
    // the target.
    let age = |file: &Path| {
    ...
    // The dependency the reading buys. The recipe writes a depfile naming
    // `hdr.h`; a build reads it, so touching `hdr.h` afterwards rebuilds the
    // target. Deferring the recipe would drop that declaration, and the build
    // after the touch would do nothing.
    let deps = tempfile::tempdir().unwrap();
    shell_named(deps.path(), "make");
    std::fs::write(
        deps.path().join("Makefile"),
        "out: .KATI_DEPFILE := out.d\nall: out\nout: src\n\t@printf 'out: hdr.h\\n' > out.d\n\t@cat src > out\n\t@echo ran >> ran.log\n",
    )
    .unwrap();
    std::fs::write(deps.path().join("src"), "srcdata\n").unwrap();
    std::fs::write(deps.path().join("hdr.h"), "one\n").unwrap();
    age(&deps.path().join("src"));
    age(&deps.path().join("hdr.h"));
    let ran = |dir: &Path| std::fs::read_to_string(dir.join("ran.log")).unwrap();

    let first = run_in(deps.path());
    assert!(first.status.success(), "{}", stderr(&first));
    assert_eq!(ran(deps.path()), "ran\n");
    let quiet = run_in(deps.path());
    assert!(quiet.status.success(), "{}", stderr(&quiet));
    assert_eq!(
        ran(deps.path()),
        "ran\n",
        "a second build with nothing changed runs nothing"
    );
    std::fs::write(deps.path().join("hdr.h"), "two\n").unwrap();
    let after = run_in(deps.path());
    assert!(after.status.success(), "{}", stderr(&after));
    assert_eq!(
        ran(deps.path()),
        "ran\nran\n",
        "touching a depfile-listed prerequisite rebuilds the target"
    );

    // The write the reading costs. A `$(file ...)` in a depfile recipe whose
    // target is up to date is performed at construction, where GNU Make — which
    // treats `.KATI_DEPFILE` as an inert variable and runs nothing — performs
    // nothing. A sound deferral would leave `written` absent, which fails this
    // and reopens the decision.
    let fresh = tempfile::tempdir().unwrap();
    shell_named(fresh.path(), "make");
    std::fs::write(
        fresh.path().join("Makefile"),
        "out: .KATI_DEPFILE := out.d\nall: out\nout: dep\n\t$(file > written,made)\n\t@echo built > out\n",
    )
    .unwrap();
    std::fs::write(fresh.path().join("dep"), "").unwrap();
    std::fs::write(fresh.path().join("out"), "built\n").unwrap();
    age(&fresh.path().join("dep"));
    let current = run_in(fresh.path());
    assert!(current.status.success(), "{}", stderr(&current));
    assert_eq!(
        std::fs::read_to_string(fresh.path().join("written")).unwrap(),
        "made\n",
        "the depfile recipe's $(file) is performed at construction"
    );
}
